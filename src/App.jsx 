...
/* --------------------------------- HEADER / NAV --------------------------------- */
function Header({ query, setQuery, onSearch, area, onChangeLocation, onBack, title, shopName }) {
  if (title) {
    return (
  ...
  return (
    <div className="sticky top-0 z-20" style={{ background: COLORS.bg, borderBottom: `1px solid ${COLORS.border}` }}>
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSearch(); }} className="px-4 py-3">
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl" style={{ background: '#fff', border: `1px solid ${COLORS.border}` }}>
...
function BottomNav({ page, nav, cartCount }) {
  const items = [
    ...
    { id: 'myorders', label: 'Orders', Icon: ClipboardList },
...
/* ----------------------------------- PAGES ----------------------------------- */
function HomePage({ products, nav, onAdd, cart, area }) {
...
function AdminLogin({ onLogin, adminPassword }) {
  ...
  return (
    <div className="flex flex-col items-center px-6 pt-20">
      ...
      {adminPassword === 'admin123' && <p style={{ fontFamily: bodyFont, fontSize: 11, color: COLORS.inkSoft, marginTop: 14 }}>First time here? Default password is admin123 &mdash; change it under Admin &rarr; Delivery once you're in.</p>}
    </div>
  );
}
...
function AdminProducts({ products, setProducts }) {
  ...
  const [form, setForm] = useState({ name: '', category: REAL_CATEGORIES[0].id, price: '', mrp: '', stock: '', emoji: '\ud83d\udecd\ufe0f', desc: '', imageUrl: '' });
  ...
  const add = async () => {
    ...
    setForm({ name: '', category: REAL_CATEGORIES[0].id, price: '', mrp: '', stock: '', emoji: '\ud83d\udecd\ufe0f', desc: '', imageUrl: '' });
    setShowAdd(false);
  };
      ...
      {showAdd && (
        <div className="rounded-2xl p-4 mb-4 flex flex-col gap-2.5" style={{ background: '#fff', border: `1px solid ${COLORS.border}` }}>
          ...
          <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="px-3 py-2.5 rounded-lg" style={{ border: `1px solid ${COLORS.border}`, fontFamily: bodyFont, fontSize: 12.5 }}>
            {REAL_CATEGORIES.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
...
function AdminPage({ products, setProducts, orders, setOrders, deliverySettings, setDeliverySettings, onLogout, adminPassword, setAdminPassword }) {
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      // Cart, saved location, and the admin password are always local to this browser,
      // regardless of whether a Supabase backend is connected.
      try {
        ...
        const [c, loc, pw] = results.map((r) => (r.status === 'fulfilled' ? r.value : null));
  ...
  const filteredForSearch = useMemo(() => {
    ...
    return products.filter((p) => p.name.toLowerCase().includes(q) || CATEGORIES.find((c) => c.id === p.category)?.name.toLowerCase().includes(q));
  }, [query, products]);
  ...
  const headerTitleMap = {
    category: CATEGORIES.find((c) => c.id === route.params.id)?.name,
      ...
      <div className="w-full flex flex-col" style={{ maxWidth: 448, minHeight: '100vh', background: COLORS.bg, boxShadow: '0 0 40px rgba(0,0,0,0.06)' }}>
        ...
        {!isAdminRoute && (
          showBackHeader ? (
            ...
            <Header query={query} setQuery={setQuery} onSearch={runSearch} area={deliveryArea} onChangeLocation={() => setShowLocationModal(true)} shopName={deliverySettings.shopName} />
          )
        )}

        <div className="flex-1">
          {route.page === 'home' && <HomePage products={products} nav={nav} onAdd={addToCart} cart={cart} area={deliveryArea} />}
          {route.page === 'categories' && <CategoriesPage nav={nav} />}
